	...
	<head>
		...
		<script>
			...
			function connectToServer() {
				socket = new WebSocket("ws://localhost:8080/csci201finalprojectfinal/ws");
				...
				var xhttp = new XMLHttpRequest();
			   	...
			   	xhttp.onreadystatechange = function() { 
			   		...
			   		if(this.readyState == 4 && this.status == 200) {
			   			...
			   			console.log(length);
			   			console.log(classEnrollmentList[0]);
			   			console.log(friendsListGlobalVar.data.length);
		    				...
		    				for(var j = 0; j < friendsListGlobalVar.data.length; j++) {
		    					...
		    					if (friendName == usernameStudent) {
		    						...
		    						var table = document.getElementById("tableFriendsList");
		    				  		var checkUser = false;
		    						for (var i = 0, row; row = table.rows[i]; i++) {
		    							//if (!usernameStudent.localeCompare(row.cells[1])) {
