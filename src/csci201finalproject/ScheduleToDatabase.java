	...
	public void pushToDatabase() {
		...
		try {
			...
			if(rs.next()) {
				...
				java.sql.PreparedStatement checkSchedule = conn.prepareStatement("SELECT * FROM Schedules WHERE userID=?");
				checkSchedule.setInt(1, userDatabaseID);
				
				ResultSet scCheck = checkSchedule.executeQuery();
				
				if(scCheck.next()) {
					java.sql.PreparedStatement deletePrevSchedule = conn.prepareStatement("DELETE FROM Schedules WHERE userID=?");
					deletePrevSchedule.setInt(1, userDatabaseID);
					deletePrevSchedule.executeUpdate();
				}
				...
				System.out.println("User was not found in the database-ScheduleToDatabase");
			}
		} catch (ClassNotFoundException e) {
			System.out.println("JDBC driver was not found! " + e.getMessage());
		} catch (SQLException e) {
			System.out.println("SQL Exception in ScheduleToDatabase. Could not connection to databse. " + e.getLocalizedMessage());
		}
	}
}